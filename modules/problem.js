...
app.get('/problems', async (req, res) => {
  try {
    ...
    if (!['id', 'title', 'rating', 'ac_num', 'submit_num', 'ac_rate', 'publicize_time'].includes(sort) || !['asc', 'desc'].includes(order)) {
      throw new ErrorMessage('錯誤的排序參數。');
    }
...
app.get('/problem/:id/export', async (req, res) => {
  try {
    let id = parseInt(req.params.id);
    let problem = await Problem.findById(id);
    if (!problem || !problem.is_public) throw new ErrorMessage('無此題目。');
...
app.get('/problem/:id/edit', async (req, res) => {
  try {
    ...
    if (!problem) {
      if (!res.locals.user) throw new ErrorMessage('請登入後繼續。', { '登入': syzoj.utils.makeUrl(['login'], { 'url': req.originalUrl }) });
...
app.post('/problem/:id/edit', async (req, res) => {
  try {
    ...
    if (!req.body.title.trim()) throw new ErrorMessage('題目名稱不能為空。');
...
app.post('/problem/:id/import', async (req, res) => {
  try {
    ...
    if (!json.success) throw new ErrorMessage('題目載入失敗。', null, json.error);

    if (!json.obj.title.trim()) throw new ErrorMessage('題目名稱不能為空。');
    ...
    if (validateMsg) throw new ErrorMessage('無效的題目資料設定。', null, validateMsg);
  ...
  try {
    ...
    if (problem.type === 'submit-answer' && req.body.type !== 'submit-answer' || problem.type !== 'submit-answer' && req.body.type === 'submit-answer') {
      if (await JudgeState.count({ problem_id: id }) !== 0) {
        throw new ErrorMessage('已有解題的題目不允許在提交答案和非提交答案之間更改。');
      }
    }
...
async function setPublic(req, res, is_public) {
  try {
    ...
    if (!allowedManage) throw new ErrorMessage('您沒有權限進行此動作。');
...
app.post('/problem/:id/submit', app.multer.fields([{ name: 'answer', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if (problem.type === 'submit-answer') {
      ...
      if (!req.files['answer']) {
        ...
        try {
          ...
          throw new ErrorMessage('無法分析提交檔案。');
        }
      } else {
      ...
      if (!file.md5) throw new ErrorMessage('上傳答案檔案失敗。');
      ...
      let code;
      if (req.files['answer']) {
        if (req.files['answer'][0].size > syzoj.config.limit.submit_code) throw new ErrorMessage('程式碼文件太大。');
        code = (await fs.readFile(req.files['answer'][0].path)).toString();
      } else {
        if (Buffer.from(req.body.code).length > syzoj.config.limit.submit_code) throw new ErrorMessage('城市馬太長。');
        code = req.body.code;
      }
    ...
    if (contest_id) {
      ...
      if ((!contest.isRunning()) && (!await contest.isSupervisior(curUser))) throw new ErrorMessage('比賽尚未開始或已結束。');
      let problems_id = await contest.getProblems();
      if (!problems_id.includes(id)) throw new ErrorMessage('無此題目。');
      ...
      if (!await problem.isAllowedUseBy(curUser)) throw new ErrorMessage('您沒有權限進行此動作。');
      judge_state.type = 0;
      await judge_state.save();
    }
    ...
    try {
      ...
      throw new ErrorMessage(`無法開始評測：${err.toString()}`);
    }
...
app.post('/problem/:id/delete', async (req, res) => {
  try {
    ...
    if (!problem.isAllowedManageBy(res.locals.user)) throw new ErrorMessage('您沒有權限進行此動作。');
...
app.get('/problem/:id/testdata/download/:filename?', async (req, res) => {
  try {
    ...
    if (!await syzoj.utils.isFile(filename)) throw new ErrorMessage('檔案不存在。');
...
app.get('/problem/:id/download/additional_file', async (req, res) => {
  try {
    ...
    if (contest_id) {
      ...
      if (!contest) throw new ErrorMessage('無此比賽。');
      if (!contest.isRunning()) throw new ErrorMessage('比賽尚未開始或已結束。');
      let problems_id = await contest.getProblems();
      if (!problems_id.includes(id)) throw new ErrorMessage('無此題目。');
    } else {
      if (!await problem.isAllowedUseBy(res.locals.user)) throw new ErrorMessage('您沒有權限進行此動作。');
...
app.get('/problem/:id/statistics/:type', async (req, res) => {
  try {
    ...
    if (count === null) throw new ErrorMessage('無此統計類型。');
...
app.post('/problem/:id/custom-test', app.multer.fields([{ name: 'code_upload', maxCount: 1 }, { name: 'input_file', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if (!problem) throw new ErrorMessage('無此題目。');
    if (!res.locals.user) throw new ErrorMessage('請登入後繼續。', { '登入': syzoj.utils.makeUrl(['login'], { 'url': syzoj.utils.makeUrl(['problem', id]) }) });
    if (!await problem.isAllowedUseBy(res.locals.user)) throw new ErrorMessage('您沒有權限進行此動作。');
